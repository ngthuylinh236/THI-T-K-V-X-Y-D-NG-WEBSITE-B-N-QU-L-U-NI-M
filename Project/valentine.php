...
$category = isset($_GET['category']) ? $_GET['category'] : 'Quà valentine';

// Truy vấn sản phẩm theo danh mục
$stmt = $conn->prepare("SELECT * FROM products WHERE product_category = ?");
$stmt->bind_param("s", $category);
$stmt->execute();
$valentine_products = $stmt->get_result();
?>

<?php include('layout/header.php'); ?>

    <!-- Sản phẩm Giáng Sinh -->
    <section id="valentine" class="my-5 pd-5">
        <div class="container text-center mt-5 py-5">
            <h3 style="font-family: 'Arial'; font-weight: 700;">Quà Valentine</h3>
            <hr style="color:coral">
            <p>Hãy khám phá những món quà Valentine tuyệt vời của chúng tôi!</p>
        </div> 
        <div class="row mx-auto container-fluid">
            <?php if ($valentine_products->num_rows > 0) : ?>
                <?php while ($row = $valentine_products->fetch_assoc()) : ?>
