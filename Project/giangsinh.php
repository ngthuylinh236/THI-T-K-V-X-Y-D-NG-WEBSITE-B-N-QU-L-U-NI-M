...
$category = isset($_GET['category']) ? $_GET['category'] : 'Quà giáng sinh';

// Truy vấn sản phẩm theo danh mục
$stmt = $conn->prepare("SELECT * FROM products WHERE product_category = ?");
$stmt->bind_param("s", $category);
$stmt->execute();
$giangsinh_products = $stmt->get_result();
?>

<?php include('layout/header.php'); ?>

    <!-- Sản phẩm Giáng Sinh -->
    <section id="giangsinh" class="my-5 pd-5">
        <div class="container text-center mt-5 py-5">
            <h3 style="font-family: 'Arial'; font-weight: 700;">Quà Giáng Sinh</h3>
            <hr style="color:coral">
            <p>Hãy khám phá những món quà Giáng sinh tuyệt vời của chúng tôi!</p>
        </div> 
        <div class="row mx-auto container-fluid">
            <?php if ($giangsinh_products->num_rows > 0) : ?>
                <?php while ($row = $giangsinh_products->fetch_assoc()) : ?>
